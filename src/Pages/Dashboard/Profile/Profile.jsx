...
const InfoRow = ({ label, value }) => (
  <div className="flex flex-col md:flex-row md:items-center md:justify-between py-3 border-b border-token">
    ...
    <span className="font-medium mt-1 md:mt-0">{value || "N/A"}</span>
  </div>
);
  ...
  <div className="w-full mx-auto mt-5 md:ml-5">
        ...
        <div className="flex flex-col sm:flex-row items-center sm:items-start gap-6 mb-6">
          <img
            ...
            alt="Profile"
            className="w-28 h-28 rounded-full object-cover border-4 border-token"
          />
          <div className="flex-1 text-center sm:text-left">
            <h1 className="text-2xl font-bold">
              {user?.displayName || userData?.name || "User"}
            </h1>
            <p className="text-muted">{user?.email || userData?.email}</p>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            <div className="r-xl p-5" style={{ background: "var(--color-surface)", border: "1px solid var(--color-border)" }}>
              <h2 className="text-lg font-semibold mb-3" style={{color: "var(--color-primary)"}}>Personal Information</h2>
            ...
            <div className="r-xl p-5" style={{ background: "var(--color-surface)", border: "1px solid var(--color-border)" }}>
              <h2 className="text-lg font-semibold mb-3" style={{color: "var(--color-primary)"}}>Address</h2>
